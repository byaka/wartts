...
processMap.init=function(apiHost){
   ...
   processMap.timer=setTimeout(function(){
      clearTimeout(processMap.timer);
      processMap.dataFromApi();
      processMap.timer=setTimeout(arguments.callee, 50);
   }, 500);
}

processMap.dataFromApi=function(){
   var url=processMap.apiHost+'/map_obj.json';
   if(processMap.apiFake) //if fakeApi enabled, we get API request from github's example
      url='https://raw.githubusercontent.com/byaka/WarThunderTacticalScreen_discuss/master/API/mapObjects.js';
   ajaxMe(url, function(data, status, r) {
      if(!data) //empty request
         return print('EMPTY_MAP');
      //parse to JSON
      var data=JSON.parse(data);
      processMap.data2group(data);
   })
}
...
processMap.data2group=function(data){
   ...
   forMe(groups, function(g){
      ...
      if(!s || s.hashCode()!=oldHash[g])
         renderingQueue_map(Object.make([[g, cloneMe(processMap.dataGroupForRender[g])]]));
   })
}
